...
const Searching = ({ isSearching, searchInputValue }) => {
    ...
    let valueToSearch = searchInputValue&&searchInputValue[0].toUpperCase() + searchInputValue.slice(1)
    ...
    const searchingField = searchCollection === 'jobs'?'title':"Candidate Name"
        ...
        let handle = setTimeout(() => {
            if (valueToSearch.length >= 3) {
                ...
                async function handleSearch() { 
                    ...
                    Array.isArray(resultData)&&!resultData.length?setSearchResults('Nothig found'):setSearchResults(resultData)
                    setLoading(false)
                }
                handleSearch()
            }
    ...
    function RenderSearchedResults({searchResults}) {
      return (
        <div className="search-result">
              {searchResults ==='Nothig found'?<div>No results found for the search.</div>:(<>
            {searchResults.map((res) => {
                return <div className="result-line">
                    <div className="avatar-circel">
                        <div className="avatar-initial">
                            <span> {res["Candidate Name"][0]}</span>
                           </div> 
                    </div>
                    <li key={uuid()}><Link to={`/candidate/${res.id}`}>
                        <span>{res["Candidate Name"]}</span>
                        <span className="can-phone">{res['Candidate Phone Number']}</span>
                    </Link>
                    </li>
                </div>;
            })}
    ...
    <div className="searchResultWrap">
      <div className="filter">
          ...
          <span>Search for</span>
              ...
              <Button
                  style={searchCollection ==='candidates'?{backgroundColor:'#1976d2', color:'white'}:{}}
          variant="outlined"
          sx={{ height: "20px" }}
        ...
        >
          Candidates
              ...
              <Button
                  style={searchCollection ==='jobs'?{backgroundColor:'#1976d2', color:'white'}:{}}
          variant="outlined"
          sx={{ height: "20px" }}
